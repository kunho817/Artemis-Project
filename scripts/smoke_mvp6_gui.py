"""Run the MVP 6 GUI e2e smoke test with an isolated writable project."""
...
def create_e2e_project(root: Path) -> Path:
    ...
    project_root.mkdir()
    (project_root / "README.md").write_text("Artemis MVP 6 GUI smoke project\n", encoding="utf-8")
    docs_dir = project_root / "docs"
    docs_dir.mkdir()
    (docs_dir / "artemis_mvp6.md").write_text(
    ...
    with tempfile.TemporaryDirectory(prefix="artemis-mvp6-gui-") as tmp_dir:
        ...
        try:
            ...
            completed = subprocess.run(
                ["npm.cmd", "run", "test:e2e", "--", "tests/mvp6-smoke.spec.ts"],
                cwd=GUI_DIR,
                env=env,
                check=False,
                capture_output=True,

...
다만 기본 observability backend는 LangSmith Cloud가 아니라 Artemis 내부 local trace store이다. LangSmith Cloud는 사용량 비용이 발생하므로 기본값으로 두지 않는다.
...
민감 정보 문제는 local-first 저장과 redaction policy로 해결한다.

---

## 2. MVP 1 한 문장 정의

**MVP 1은 사용자의 자연어 요청을 Work Package로 구조화하고, 그 과정의 상태, 이벤트, 승인 대기, trace correlation을 남기는 backend foundation이다.**
...
Artemis local trace store가 기본 추적 계층이다.
...
Control Plane의 Event와 Agent Backend의 trace는 correlation id로 연결한다.
...
project_id
trace_id
optional_external_trace_id
```

LangSmith는 Cloud 기본값이 아니다. 필요할 경우 self-hosted LangSmith endpoint 또는 명시적 opt-in Cloud endpoint로 연결할 수 있지만, MVP 1의 기본 완료 조건은 local trace correlation이다.
...
9. local trace correlation id를 확인할 수 있다.
...
MVP 1의 목표는 Control Plane과 Agent Backend를 분리한 상태에서 사용자 요청을 Work Package로 변환하고, 이벤트/상태/승인 대기/local trace correlation까지 남기는 backend foundation vertical slice를 구현하는 것이다.
...
- Observability는 기본값이며, 기본 backend는 Artemis local trace store이다.
- LangSmith Cloud는 기본값이 아니며, self-hosted/Cloud endpoint 연동은 명시적 설정으로만 사용한다.
```
